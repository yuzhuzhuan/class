...
export default class extends Vue {
   ...
   * 水平还是垂直
   */
  @Prop({ default: EType.horizontal }) type!: EType;
  /**
   * 父级的id
   */
   ...
   * 滚动条的背景颜色
   */
  @Prop({ default: 'rgb(66, 59, 54)' }) bgColor!: string;
   ...
   * 计算父级的宽/高
   */
  caculateParentLength() {
    this.parentLength =
      this.type === EType.horizontal ? this.PDom.clientWidth : this.PDom.clientHeight;
  }

  /**
   * 处理水平状态的滚动条
   */
  horizontalScrollTo(eventDelta: number) {
    const scrollContainerLeft = parseFloat(window.getComputedStyle(this.scrollContainerDom).left);
    ...
    if (
      ...
      let moveValue =
        scrollContainerLeft + eventDelta / 4 > 0 ? 0 : scrollContainerLeft + eventDelta / 4;
      moveValue =
    ...
    if (scrollLeft >= 0 || scrollLeft + this.scrollDom.clientWidth <= this.PDom.clientWidth) {
      let moveValue =
  ...
  mounted() {
    ...
    // 滚动条容器的dom
    this.scrollContainerDom = this.$el.querySelectorAll('.scroller-container')[0] as HTMLDivElement;
    // 计算子元素的长度/宽度
