...
export default class extends Vue {
  ...
  onChange (value: string) {
   ...
   initOpts = {
    ...
    //       : "http://192.168.0.21:8888/";
    //   let {
    //     data: { data },
    //   } = await api.fileList({ path: categoryName });
    //   this.changeData(data); // 把有子级的文件夹放最上面

    //   let name = data
    //     .filter(
    //       (item: any) => item.directory && item.name != "site_resources/assets/"
    //     )
    //     .map((item: any) => {
    //       return item.name;
    //     });
    ...
    //   //------------------------------------
    //   let arr = data
    //     .filter((item: any) => item.name != "site_resources/assets/")
    //     .map((item: any) => {
    //       if (item.directory) {
    //         return {
    //           title: item.name,
    //           menu: newMenu.filter((i: any) =>
    //             i.parentName.startsWith(item.name)
    //           ),
    //         };
    //       } else {
    //         return {
