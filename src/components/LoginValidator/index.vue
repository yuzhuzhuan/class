...
export default class extends Vue {
  ...
  public mouseMoveFn (e: MouseEvent) {
    if (this.mouseMoveState) {
      ...
      if (width > 0 && width <= this.maxWidth) {
        const handerDom: NodeListOf<HTMLDivElement> =
          document.querySelectorAll('.handler');
        handerDom[0].style.left = width + 'px';
        const dragDom: NodeListOf<HTMLDivElement> =
          document.querySelectorAll('.drag_bg');
  ...
  public moseUpFn (e: MouseEvent) {
    ...
    if (width < this.maxWidth) {
      const handlerDom: NodeListOf<HTMLDivElement> =
        document.querySelectorAll('.handler');
  ...
  public successFunction () {
    ...
    // 给图标赋值
    (
      document.querySelectorAll('.drag_text') as NodeListOf<HTMLDivElement>
    )[0].style.color = '#fff';
    (
      document.querySelectorAll('.handler') as NodeListOf<HTMLDivElement>
    )[0].style.left = this.maxWidth + 'px';
    (
      document.querySelectorAll('.drag_bg') as NodeListOf<HTMLDivElement>
    )[0].style.width = this.maxWidth + 'px';
  }
